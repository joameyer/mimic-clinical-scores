...
from mimic_clinical_scores.scores.saps_iii_adapted.reference import physiology_points, total_and_probabilities
from mimic_clinical_scores.scores.saps_iii_adapted.specification import SAPSIII_ADAPTED_SPEC, load_itemid_manifest


def test_adapted_item_manifest_is_complete_and_versioned() -> None:
    manifest = load_itemid_manifest()
    assert manifest["manifest_version"] == "saps-iii-adapted-v1"
...
def test_original_physiology_cutoffs_and_equations() -> None:
    points = physiology_points(
        ...
        bilirubin_mg_dl=6, creatinine_mg_dl=3.5, wbc_k_ul=15,
    ...
    score, global_probability, na_probability = total_and_probabilities(points)
    assert score == 98
    ...
    base = dict(
        ...
        bilirubin_mg_dl=1, creatinine_mg_dl=1, wbc_k_ul=10,
    ...
    assert values("wbc_k_ul", [14.9,15], "wbc_score") == [0,2]
    ...
    missing = physiology_points(**{**base, **{
        ...
        "bilirubin_mg_dl": None, "creatinine_mg_dl": None, "wbc_k_ul": None,
        "platelets_k_ul": None, "ph": None, "pao2_mm_hg": None,
    }})
    assert set(missing.values()) == {None}
    assert total_and_probabilities(missing)[0] == 16
...
def test_filtered_admission_window_and_score(tmp_path, project_root) -> None:
    ...
    def chart(itemid, hour, value):
        data["icu/chartevents.csv.gz"].append(row("icu/chartevents.csv.gz", subject_id=1, hadm_id=10, stay_id=100, caregiver_id=1, charttime=ts(base, hours=hour), storetime=ts(base, hours=hour), itemid=itemid, value=str(value), valuenum=value))
    chart(220045, -1, 160); chart(220045, 1, 159); chart(220045, 1.001, 300)
    chart(220179, 0, 69); chart(223762, 0, 34)
    ...
    for itemid, value in ((50885,6),(50912,3.5),(51301,15),(51265,19),(50820,7.25),(50821,50)):
        lab(itemid, value)
    ...
    result = con.execute("SELECT gcs_score,hr_score,sysbp_score,temp_score,bilirubin_score,creatinine_score,wbc_score,platelet_score,ph_score,oxygenation_score,vasoactive_score,saps_iii_adapted,saps_iii_prob_global_adapted,saps_iii_prob_north_america_adapted FROM mimiciv_derived.saps_iii_adapted").fetchone()
    reference = physiology_points(gcs=4, heart_rate=160, systolic_bp=69, temperature_c=34, bilirubin_mg_dl=6, creatinine_mg_dl=3.5, wbc_k_ul=15, platelets_k_ul=19, ph=7.25, mechanically_ventilated=True, pao2_mm_hg=50, pf_ratio=100)
    assert result[:10] == tuple(reference.values())
    assert result[10] == 3
    context = con.execute(
        "SELECT age_score,hospital_los_score,admission_location_score,comorbidity_score,"
        "vasoactive_score,planned_icu_score,admission_reason_score,surgery_status_score,"
        "surgical_site_score,infection_score FROM mimiciv_derived.saps_iii_adapted"
    ).fetchone()
    ...
    assert exported == ("saps-iii-adapted-v1", True, False)
    con.close()
